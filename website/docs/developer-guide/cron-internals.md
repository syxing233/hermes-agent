...
Jobs are stored in `${HERMES_HOME}/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ${HERMES_HOME}/scripts/check_competitors.py
...
| Local file | `local` | Save to `${HERMES_HOME}/cron/output/` |
