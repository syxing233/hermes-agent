...
Hermes Agent uses a SQLite database (`${HERMES_HOME}/state.db`) to persist session
...
db = SessionDB()                           # Default: ${HERMES_HOME}/state.db
...
`${HERMES_HOME}/` by default, or the value of `HERMES_HOME` environment variable.
