...
|--------|-----------------|
| `${HERMES_HOME}/.env` | API keys, bot tokens, platform credentials |
| `${HERMES_HOME}/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `${HERMES_HOME}/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- PID file at `${HERMES_HOME}/gateway.pid` — profile-scoped process tracking
