...
After `nix profile install`, `hermes`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `hermes setup` walks you through provider selection, `hermes gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `${HERMES_HOME}/`.
...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `${HERMES_HOME}/` directory.
:::
...
{ config, ... }: {
  services.hermes-agent = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    documents = {
      "SOUL.md" = builtins.readFile /home/user/.hermes-home/SOUL.md;
...
```bash
hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ${HERMES_HOME}/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/hermes/.hermes-home/mcp-tokens/
...
${HERMES_HOME} -> /var/lib/hermes/.hermes       (symlink bridge, per hostUsers)
...
```bash
# Native mode
sudo -u hermes cat /var/lib/hermes/.hermes-home/.env

# Container mode
docker exec hermes-agent cat /data/.hermes-home/.env
```
