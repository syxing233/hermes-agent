...
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `${HERMES_HOME}/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `${HERMES_HOME}/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `${HERMES_HOME}/.env` manually for API keys.
:::
...
:::tip
The record key is configurable via `voice.record_key` in `${HERMES_HOME}/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `${HERMES_HOME}/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `${HERMES_HOME}/.env`
...
- Review gateway logs: `tail -f ${HERMES_HOME}/logs/gateway.log`
