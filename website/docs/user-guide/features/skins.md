...
/skin mytheme        # switch to a custom skin from ${HERMES_HOME}/skins/mytheme.yaml
```

Or set the default skin in `${HERMES_HOME}/config.yaml`:
...
Create YAML files under `${HERMES_HOME}/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ${HERMES_HOME}/skins/mytheme.yaml
...
- Saves directly to `${HERMES_HOME}/skins/`
- Activates a skin by updating `${HERMES_HOME}/config.yaml`
...
6. Click **Save** to write the skin YAML to `${HERMES_HOME}/skins/`.
...
- User skins in `${HERMES_HOME}/skins/` take precedence over built-in skins with the same name.
