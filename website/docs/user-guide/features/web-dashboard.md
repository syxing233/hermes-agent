...
Browse, search, and toggle skills and toolsets. Skills are loaded from `${HERMES_HOME}/skills/` and grouped by category.
...
This re-reads `${HERMES_HOME}/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
