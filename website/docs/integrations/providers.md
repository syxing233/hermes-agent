...
| **OpenRouter** | `OPENROUTER_API_KEY` in `${HERMES_HOME}/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `${HERMES_HOME}/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `${HERMES_HOME}/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `${HERMES_HOME}/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `${HERMES_HOME}/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `${HERMES_HOME}/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **MiniMax** | `MINIMAX_API_KEY` in `${HERMES_HOME}/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `${HERMES_HOME}/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `${HERMES_HOME}/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `${HERMES_HOME}/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `${HERMES_HOME}/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `${HERMES_HOME}/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `${HERMES_HOME}/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `${HERMES_HOME}/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `${HERMES_HOME}/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `${HERMES_HOME}/.env` (provider: `gemini`) |
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Hermes stores the resulting credentials in its own auth store under `${HERMES_HOME}/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
When you choose Anthropic OAuth through `hermes model`, Hermes prefers Claude Code's own credential store over copying the token into `${HERMES_HOME}/.env`. That keeps refreshable Claude credentials refreshable.
...
# Requires: GLM_API_KEY in ${HERMES_HOME}/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
hermes chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ${HERMES_HOME}/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
hermes chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ${HERMES_HOME}/.env

# MiniMax (global endpoint)
hermes chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ${HERMES_HOME}/.env

# MiniMax (China endpoint)
hermes chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ${HERMES_HOME}/.env

# Alibaba Cloud / DashScope (Qwen models)
hermes chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ${HERMES_HOME}/.env

# Xiaomi MiMo
hermes chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ${HERMES_HOME}/.env

# Arcee AI (Trinity models)
hermes chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ${HERMES_HOME}/.env
```
...
# Requires: HF_TOKEN in ${HERMES_HOME}/.env
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `${HERMES_HOME}/config.yaml`:
