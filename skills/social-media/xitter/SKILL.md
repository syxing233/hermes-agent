...
If you already keep your X credentials in `${HERMES_HOME}/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ${HERMES_HOME}/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `${HERMES_HOME}/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
