...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "${HERMES_HOME:-$HOME/.hermes-home}/.env" ] && grep -q "^GITHUB_TOKEN=" "${HERMES_HOME:-$HOME/.hermes-home}/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "${HERMES_HOME:-$HOME/.hermes-home}/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
